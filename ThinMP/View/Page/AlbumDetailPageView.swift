        ...
        GeometryReader { geometry in
            VStack {
                ZStack(alignment: .top) {
                    CustomNavigationBarView(id: self.albumDetail.persistentId, primaryText: self.albumDetail.title, secondaryText: self.albumDetail.artist, side: geometry.size.width, pageRect: self.$pageRect, headerRect: self.$headerRect)
